...
async def _send_voice_safe(bot: Bot, chat_id: int, audio_bytes: bytes | None, filename: str, title: str):
    """ارسال پیام صوتی؛ اگر ساخت صدا شکست بخورد یا کاربر مسدود کرده باشد، بازی نباید متوقف شود."""
    if not audio_bytes:
        return
    try:
...
async def cb_solo_roll(callback: CallbackQuery, bot: Bot):
    ...
    if result["won"]:
        ...
        # 🎙 پیام صوتی تبریک
        audio = vm.win_voice_bytes(name)
        await _send_voice_safe(bot, callback.message.chat.id, audio, "win.mp3", "🎉 پیام تبریک")
...
async def cb_pvp_roll(callback: CallbackQuery, bot: Bot):
    ...
    if result["won"]:
        ...
        # 🎙 پیام صوتی: تبریک برای برنده، دلداری برای بازنده‌ها
        winner_name = names[slot - 1]
        win_audio = vm.win_voice_bytes(winner_name)
        await _send_voice_safe(bot, current_player_id, win_audio, "win.mp3", "🎉 پیام تبریک")

        for i, uid in enumerate(players, 1):
            if uid == current_player_id:
                continue
            loser_name = names[i - 1]
            lose_audio = vm.lose_voice_bytes(loser_name)
            await _send_voice_safe(bot, uid, lose_audio, "lose.mp3", "🎮 نتیجه بازی")
